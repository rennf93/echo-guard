...
func newRequestShim(c echolib.Context, maxBodyBytes int64) *requestShim {
	if maxBodyBytes <= 0 {
		maxBodyBytes = DefaultMaxBodyBytes
	}
	req := c.Request()
	shim := &requestShim{req: req, maxBytes: maxBodyBytes}
	if req.Body != nil {
		shim.origBody = req.Body
		shim.source = req.Body
		req.Body = &replayBody{shim: shim}
	}
	if routeID, ok := req.Context().Value(routeIDContextKey{}).(string); ok {
